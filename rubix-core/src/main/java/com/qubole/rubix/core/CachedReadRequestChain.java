...
public class CachedReadRequestChain extends ReadRequestChain
{
  ...
  public Integer call() throws IOException
  {
    ...
    try {
      ...
      log.error(String.format("Fall back to read from object store for %s . Could not read data from cached file due to %s:", localCachedFile, ex.getMessage()), ex.getCause());
      needsInvalidation = true;
      directDataRead = readFromRemoteFileSystem();
      return directDataRead;
    }
